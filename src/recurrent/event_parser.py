...
RE_EVENT = r"(?P<event>(?:every|each|\bon\b|\bthe\b|repeat|%s|%s|%s)(?:s|ing)?(.*))" % (
    RE_DAILY.pattern,
    RE_PLURAL_WEEKDAY.pattern,
    RE_ORDINAL_NOT_ANCHORED.pattern,
)
RE_EVENT_NO_ORD = (
    r"(?P<event>(?:every|each|\bon\b|\bthe\b|repeat|%s|%s)(?:s|ing)?(.*))"
...
RE_EXCEPT = re.compile(
    r"(?P<event>.*?)\bexcept(?:\s+for\s|\s+on\s+|\s+in\s+)?(?P<except>.*)$"
)
RE_YEAR = re.compile(r"\b(\d\d\d\d)\b")
RE_ORD_DAY_WEEK_MONTH_OR_YEAR = re.compile(
    r"(?P<ord>%s)\s+(?P<unit>(?:%s|day|week|month|year)\b)"
    % (RE_ORDINAL_NOT_ANCHORED.pattern, RE_DOW.pattern)
)
RE_THRU = re.compile(
...
class RecurringEvent(object):
    ...
    def __init__(
        ...
        if isinstance(now_date, datetime.date) and not isinstance(
    ...
    def increment_date(self, d, amount, units="years"):
        ...
        if units == "years":
            try:
                ...
                return d + (
                    datetime.date(d.year + amount, 1, 1) - datetime.date(d.year, 1, 1)
                )
        elif units == "months":
            years = 0
            ...
            try:
                ...
                return d + (
                    datetime.date(d.year + years, month, 1)
                    - datetime.date(d.year, month, 1)
                )
        else:
            multiplier = 1
            if units == "weeks":
    ...
    def parse_start_and_end(self, s):
        ...
        if m:
            ...
            r = RecurringEvent(
                now_date=self.now_date, preferred_time_range=self.preferred_time_range
            )
            rfc = r.parse(exc)
    ...
    def extract_ending(self, s):
        ...
        if m:
            self.until = self.parse_date(m.group("ending"))
            if (
                self.dtstart and self.until < self.dtstart
            ):  # e.g. starting Nov until Jun
                self.until = self.increment_date(self.until, 1)
    ...
    def extract_exdates(self, s):
        ...
        for d_str in s_split:
            ...
            if m:
                ...
                if (
                    not rest or y or not rest[0].isdigit()
                ):  # e.g. may; may 2020; may would work, but not may 1
                    if y:
                    ...
                    while True:
                        ...
                        if isinstance(ex, datetime.datetime):
                            ...
                            if r.month == ex[0] and (ex[1] is None or r.year == ex[1]):
                                ...
                                new_exdate.append(r)
                            if ex[1] is not None and (
                                r.year > ex[1] or (r.year == ex[1] and r.month > ex[0])
                            ):
                                ndx += 1
                                if ndx >= len(exdate):
                                    break
    ...
    def eat_times(self, tokens):  # Issue #13
        ...
        for i in range(len(tokens)):
            if tokens[i].type_ == "number" and (
                (i + 1 < len(tokens) and tokens[i + 1].type_ == "ampm")
                or (
                    i != 0
                    and tokens[i - 1].type_ == "sep"
                    and tokens[i - 1].text == "at"
                )
            ):
    ...
    def fixup_ord_intervals(self, s):
        def ord_sub(
            ...
            if unit == "day" or unit == "week" or unit == "month" or unit == "year":
                ...
                unit = " and ".join(
                    [plural_day_names[u].lower() for u in get_DoW(unit)]
                )
            return f"{ordx} {unit}"
    ...
    def process_thru(self, s):
        ...
        def sub_thru(m):
            ...
            dn = (
                plural_day_names
                if first.endswith("s") or second.endswith("s")
                else day_names
            )
            first = get_DoW(first)
    ...
    def parse_event(self, s):
        ...
        tokens = [
            t for t in tokens if t.type_ in [x[0] for x in Tokenizer.CONTENT_TYPES]
        ]
        if not tokens:
            return False
                ...
                if tokens[index].type_ == "number":
                    ...
                    if text == "day" or text == "week":  # Day or week of month/year
                        ...
                        while True:
                            if (
                                index + 1 < len(tokens)
                                and tokens[index + 1].type_ == "number"
                            ):
                                index += 1
                                got_some = True
                                n = get_number(tokens[index].text)
                                if n is not None:
                                    if text == "day":
                                        if self.freq == "monthly" or (
                                            self.freq == "yearly" and self.bymonth
                                        ):
                    ...
                    if (
                        tokens[index].text != "day"
                        or (
                            self.freq != "weekly"
                            and self.freq != "monthly"
                            and self.freq != "yearly"
                        )
                    ):  # Issue #18: Handle every year on the 40th day; every month on the 20th day
                    ...
                    # november)
                    while (
                        index + 1 < len(tokens) and tokens[index + 1].type_ == "ordinal"
                    ):
                    ...
                    if (
                        index + 2 < len(tokens)
                        and tokens[index + 1].type_ == "unit"
                        and tokens[index + 1].text == "day"
                        and tokens[index + 2].type_ == "unit"
                    ):  # Issue #18
                        index += 1  # Issue #18: Handle "first day of month" or "last day of month"

                    if index + 1 < len(tokens) and (
                        tokens[index + 1].type_ == "DoW"
                        or tokens[index + 1].type_ == "plural_weekday"
                    ):
                        ...
                        index + 1 < len(tokens) and tokens[index + 1].type_ == "number"
                    ):
                    ...
                    if index + 1 < len(tokens) and tokens[index + 1].type_ == "unit":
                        ...
                        if self.freq == "monthly":
                            ...
                            self.weekdays.extend(
                                [ordered_weekday_codes[i % 8] for i in ords]
                            )
                        index += 1
                        if index >= len(tokens):
                            break
                    elif (
                        index + 1 < len(tokens) and tokens[index + 1].type_ == "MoY"
                    ):  # 2nd of Mar
                        if not self.freq:
                            self.freq = "yearly"
                        ...
                        index + 1 < len(tokens)
                        and tokens[index + 1].type_ == "instances"
                    ):  # 3rd instance of ...
                        self.bysetpos.extend([str(i) for i in ords])
                        index += 1
                    elif (
                        self.freq is not None
                    ):  # Already have the freq, e.g. every month on the 4th
                        if self.freq == "monthly":
                            self.bymonthday.extend([str(i) for i in ords])
                        elif self.freq == "yearly":
                            self.byyearday.extend([str(i) for i in ords])
                        ...
                        len(ords) == 1 and ords[0] == 2
                    ):  # Issue #16: 'second' is ambiguous - treat here as unit
                        self.freq = "secondly"  # Issue #16
                elif (
                    tokens[index].type_ == "DoW"
                    or tokens[index].type_ == "plural_weekday"
                ):
                    ...
                    while True:
                        if (
                            index + 1 < len(tokens)
                            and tokens[index + 1].type_ == "ordinal"
                        ):  # Aug 1st
                            self.bymonthday.append(
                                str(get_ordinal_index(tokens[index + 1].text))
                            )
                            index += 1
                            if index >= len(tokens):
                                break
    ...
    def get_hour(self, hr, mod):
        ...
        if hr > 12:
            return hr
        if hr == 0:
            return 0  # ignore preferred_time_range when 0, 0 is never 12:00
    ...
    def format(self, rrule_or_datetime):
        ...
        if isinstance(rrule_or_datetime, datetime.date):  # date or datetime
            if not isinstance(
                rrule_or_datetime, datetime.datetime
            ) or rrule_or_datetime.time() == datetime.time(0):
                return rrule_or_datetime.strftime("%a %b %d, %Y").replace(" 0", " ")
            elif rrule_or_datetime.minute == 0 and rrule_or_datetime.second == 0:
                return (
                    rrule_or_datetime.strftime("%a %b %d, %Y %I%p")
                    .replace(" 0", " ")
                    .replace("AM", "am")
                    .replace("PM", "pm")
                )
                ...
                return (
                    rrule_or_datetime.strftime("%a %b %d, %Y %I:%M%p")
                    .replace(" 0", " ")
                    .replace("AM", "am")
                    .replace("PM", "pm")
                )
                ...
                return (
                    rrule_or_datetime.strftime("%a %b %d, %Y %I:%M:%S%p")
                    .replace(" 0", " ")
                    .replace("AM", "am")
                    .replace("PM", "pm")
                )
        ...
        def byday_squasher(s):
            ...
            re_squasher = re.compile(
                r"((?:\d.. to the )?last|\d..)\s(Mon|Tue|Wed|Thu|Fri|Sat|Sun) and ((?:\d.. to the )?last|\d..)\s\2"
            )
            while True:
        ...
        try:
            ...
            def add_time(rr):
                ...
                if byminute == 0:
                    return " at" + tm.strftime(" %I%p").replace(" 0", " ").replace(
                        "AM", "am"
                    ).replace("PM", "pm")
                return " at" + tm.strftime(" %I:%M%p").replace(" 0", " ").replace(
                    "AM", "am"
                ).replace("PM", "pm")
            ...
            def add_bysetpos(rr, add=" "):
                ...
                return (
                    add
                    + "for the "
                    + list_handler(number_suffix, bysetpos)
                    + " instance of "
                )
            ...
            if fr == "YEARLY":
                ...
                if bymonthday is not None and bymonth is not None:
                    ...
                    result += (
                        " "
                        + list_handler(month_name, bymonth, " or ")
                        + " "
                        + list_handler(number_suffix, bymonthday)
                    )
                    ...
                    if result.endswith(" year"):
                        ...
                        result += (
                            " "
                            + byday_squasher(list_handler(byday_name, byday))
                            + " in "
                            + list_handler(month_name, bymonth, " or ")
                        )
                    else:
                    ...
                    result += add_bysetpos(rr)
                    result += (
                        " on the " + list_handler(number_suffix, byyearday) + " day"
                    )
                    return result + add_suffix(pr)
                ...
                if byday is not None:
                    result = (
                        ...
                        + add_bysetpos(rr)
                        + " on "
                        + list_handler(byday_name, byday)
                        .replace("Sat and Sun", "weekend")
                        .replace("Mon and Tue and Wed and Thu and Fri", "weekday")
                        + add_suffix(pr)
                    )
